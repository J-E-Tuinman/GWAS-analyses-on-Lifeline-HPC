...
FILE2="/groups/umcg-lifelines/tmp02/projects/ov23_0782/jtuinman/references/ldsc/CAD_fixed.ldsc.txt"
...
python $LDSC_DIR/ldsc.py \
    --rg logCAC.clean.sumstats.gz,CAD.clean.sumstats.gz \
    --ref-ld-chr eur_w_ld_chr/ \
    --w-ld-chr eur_w_ld_chr/ \
    --out log_vs_CAD_rg
...
cp logCAC_h2* $OUTDIR/
cp CAD_h2* $OUTDIR/
cp log_vs_CAD_rg* $OUTDIR/
cp logCAC.clean.sumstats.gz $OUTDIR/
cp CAD.clean.sumstats.gz $OUTDIR/
cp $FILE1 $OUTDIR/
cp $FILE2 $OUTDIR/
