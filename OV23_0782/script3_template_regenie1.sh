...
#SBATCH --output=/groups/umcg-lifelines/tmp02/projects/ov23_0782/jtuinman/UGLI0-3/output/DATA-regenie1/logs/log3_DATA_PHENO.txt
#SBATCH --error=/groups/umcg-lifelines/tmp02/projects/ov23_0782/jtuinman/UGLI0-3/output/DATA-regenie1/logs/log3_DATA_PHENO.err
#SBATCH -t 00:40:00
...
regenie \
  ...
  --phenoFile dataF_DATA_PHENO.txt \
  --phenoCol PHENO \
  --covarFile dataF_DATA_PHENO.txt \
  --covarColList age,PC1,PC2,PC3,PC4,PC5,PC6,PC7,PC8,PC9,PC10 \
  --catCovarList gender\
  --bsize 1000 \
  --lowmem \
  --lowmem-prefix ${DIR_TEMP}/temp_pred_PHENO \
  --threads 8 \
  --maxCatLevels 99 \
  --gz \
  --out DATA-regenie1/PHENO

cd /groups/umcg-lifelines/tmp02/projects/ov23_0782/jtuinman/UGLI0-3/output/DATA-PLINK
